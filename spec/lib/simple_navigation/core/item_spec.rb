...
describe SimpleNavigation::Item do
  ...
  describe 'autogenerated_item_class' do
    context 'calls' do
      before(:each) do
        @class_generator = stub(:class_generator)
        SimpleNavigation.config.stub!(:class_generator => @class_generator)
      end
      it "should call the configured generator with the key as param" do
        @class_generator.should_receive(:call).with(:my_key)
        @item.send(:autogenerated_item_class)
      end
    end
    context 'default generator' do
      it {@item.send(:autogenerated_item_class).should == nil}
    end
  end
